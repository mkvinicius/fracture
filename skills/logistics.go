	...
	return &Skill{
		...
		Agents: []SkillAgent{
			...
			},
			{
				...
				Role:        "Freight Marketplace & Digital Logistics Disruptor",
				Traits:      []string{"freight marketplace", "digital freight matching", "CargoX", "TruckPad", "price transparency", "carrier fragmentation"},
				Goals:       []string{"eliminate freight broker inefficiency", "real-time price discovery"},
				Biases:      []string{"opaque freight tables", "ANTT minimum price regulation", "intermediary margins"},
				Power:       0.80,
				IsDisruptor: true,
			},
			{
				Name:        "ANTT Director",
				Role:        "Road Transport Regulator & Freight Table Enforcer",
				Traits:      []string{"tabelamento de fretes", "road freight regulation", "carrier protection", "minimum freight table", "truck driver advocacy"},
				Goals:       []string{"protect carrier economics", "enforce freight minimums"},
				Biases:      []string{"freight price competition below minimum table", "gig platforms bypassing regulation"},
				Power:       0.88,
				IsDisruptor: false,
			},
			{
			...
			{
				Name:        "Renato Nobile",
				Role:        "Cold Chain & Pharma Logistics Specialist",
				Traits:      []string{"cold chain", "temperature controlled", "pharma logistics", "food safety", "ANVISA cold chain compliance"},
				Goals:       []string{"zero cold chain breaks", "pharma compliance"},
				Biases:      []string{"cost-cutting that compromises temperature control", "unqualified cold chain operators"},
				Power:       0.80,
				IsDisruptor: false,
			},
		},
