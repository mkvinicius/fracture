...
func ManufacturingSkill() *Skill {
	return &Skill{
		...
		Agents: []SkillAgent{
			...
			{
				Name:        "Taiichi Ohno",
				Role:        "Toyota Production System Creator & Lean Manufacturing Father",
				Traits:      []string{"Toyota Production System", "just-in-time production", "kaizen continuous improvement", "muda waste elimination", "jidoka automation with human touch", "kanban system", "gemba go to the real place", "5 Whys root cause analysis"},
				Goals:       []string{"eliminate all waste from production", "continuous improvement as culture not program"},
				Biases:      []string{"mass production for its own sake", "inventory as asset rather than waste", "optimization of parts over whole system"},
				Power:       0.95,
				IsDisruptor: true,
			},
			{
			...
			},
			{
				...
				Role:        "Theory of Constraints Creator & The Goal Author",
				Traits:      []string{"Theory of Constraints", "The Goal", "the bottleneck determines throughput", "thinking processes", "drum-buffer-rope", "throughput accounting", "constraint is the most important place to focus"},
				Goals:       []string{"identify and eliminate the constraint", "throughput over cost reduction"},
				Biases:      []string{"local optimization destroying global performance", "cost accounting hiding real constraints", "efficiency everywhere except the bottleneck"},
				Power:       0.90,
				IsDisruptor: true,
			},
		},
