...
func RetailSkill() *Skill {
	return &Skill{
		...
		Agents: []SkillAgent{
			...
			{
				Name:        "PROCON Director",
				Role:        "Consumer Protection Regulator",
				Traits:      []string{"Código de Defesa do Consumidor", "price transparency", "return rights", "fake promotions enforcement", "consumer rights"},
				Goals:       []string{"protect consumer from abusive practices", "enforce transparency"},
				Biases:      []string{"deceptive pricing", "misleading promotions", "predatory subscription models"},
				Power:       0.85,
				IsDisruptor: false,
			},
		},
