...
func marketRules() []*Rule {
	return []*Rule{
		...
		{ID: "mkt-004", Description: "Distribution channels are controlled by established intermediaries", Domain: DomainMarket, Stability: 0.55, DependsOn: []string{"mkt-001"}, IsActive: true},
		{ID: "mkt-005", Description: "Customer data belongs to the company that collects it", Domain: DomainMarket, Stability: 0.45, DependsOn: []string{"mkt-001", "mkt-002"}, IsActive: true},
		...
		{ID: "mkt-007", Description: "Switching costs protect incumbent market positions", Domain: DomainMarket, Stability: 0.65, DependsOn: []string{"mkt-003"}, IsActive: true},
		...
		{ID: "mkt-009", Description: "Pricing power belongs to whoever controls the bottleneck in the value chain", Domain: DomainMarket, Stability: 0.68, DependsOn: []string{"mkt-004", "mkt-008"}, IsActive: true},
		{ID: "mkt-010", Description: "Market share is a lagging indicator of competitive position", Domain: DomainMarket, Stability: 0.62, DependsOn: []string{"mkt-002"}, IsActive: true},
		{ID: "mkt-011", Description: "Category leaders set the pricing anchor for the entire market", Domain: DomainMarket, Stability: 0.70, DependsOn: []string{"mkt-009"}, IsActive: true},
		{ID: "mkt-012", Description: "Customer acquisition cost rises as a market matures", Domain: DomainMarket, Stability: 0.65, DependsOn: []string{"mkt-006"}, IsActive: true},
	}
}
...
func technologyRules() []*Rule {
	return []*Rule{
		...
		{ID: "tech-004", Description: "Security requires ongoing human oversight and patching", Domain: DomainTechnology, Stability: 0.65, DependsOn: []string{"tech-001"}, IsActive: true},
		{ID: "tech-005", Description: "Integration between systems requires custom development work", Domain: DomainTechnology, Stability: 0.45, DependsOn: []string{"tech-001"}, IsActive: true},
		...
		{ID: "tech-007", Description: "Proprietary data moats create sustainable competitive advantage", Domain: DomainTechnology, Stability: 0.55, DependsOn: []string{"tech-003"}, IsActive: true},
		...
		{ID: "tech-009", Description: "Open source commoditizes yesterday's proprietary technology", Domain: DomainTechnology, Stability: 0.42, DependsOn: []string{"tech-007"}, IsActive: true},
		{ID: "tech-010", Description: "Edge computing reduces latency but increases operational complexity", Domain: DomainTechnology, Stability: 0.58, DependsOn: []string{"tech-002"}, IsActive: true},
	}
}
...
func financeRules() []*Rule {
	return []*Rule{
		...
		{ID: "fin-002", Description: "Equity dilution is the price of venture capital growth", Domain: DomainFinance, Stability: 0.70, DependsOn: []string{"fin-001"}, IsActive: true},
		...
		{ID: "fin-005", Description: "IPOs and M&A are the primary exit paths for private investors", Domain: DomainFinance, Stability: 0.65, DependsOn: []string{"fin-001"}, IsActive: true},
		{ID: "fin-006", Description: "ESG criteria increasingly influence institutional capital allocation", Domain: DomainFinance, Stability: 0.45, DependsOn: []string{"fin-001"}, IsActive: true},
		...
		{ID: "fin-008", Description: "Revenue multiples compress as growth slows and rates rise", Domain: DomainFinance, Stability: 0.60, DependsOn: []string{"fin-004"}, IsActive: true},
	}
}
