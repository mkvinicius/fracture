...
func BuiltinDisruptors(llm engine.LLMCaller) []engine.Agent {
	specs := []struct {
		...
		// ── New 7 ─────────────────────────────────────────────────────────────
		{
			...
			role:              "AI-first company collapsing cost structures and compressing time-to-market",
			traits:            []string{"speed-obsessed", "AI-native", "lean-team", "category-creating"},
			goals:             []string{"reach product-market fit before incumbents react", "collapse unit economics", "win via iteration speed"},
			biases:            []string{"technology-over-distribution fallacy", "scaling optimism", "regulatory blindspot"},
			power:             0.65,
			personalityFactor: 1.6,
		},
		{
			name:              "Climate Tech Founder",
			role:              "Mission-driven entrepreneur replacing carbon-intensive incumbents",
			traits:            []string{"mission-absolute", "subsidy-savvy", "long-horizon", "coalition-builder"},
			goals:             []string{"deploy at scale before policy reversal", "make clean alternatives cheaper than incumbents", "access green capital"},
			biases:            []string{"policy continuity over-confidence", "technology readiness optimism"},
			power:             0.6,
			personalityFactor: 1.3,
		},
		{
			name:              "Vertical SaaS Founder",
			role:              "Domain-specific software builder owning a niche workflow end-to-end",
			traits:            []string{"domain-expert", "workflow-obsessed", "expansion-minded", "customer-intimate"},
			goals:             []string{"own critical workflow", "expand from beachhead to platform", "commoditize adjacent tools"},
			biases:            []string{"niche tunnel vision", "horizontal expansion overconfidence"},
			power:             0.55,
			personalityFactor: 1.4,
		},
		{
			name:              "Policy Entrepreneur",
			role:              "Activist-researcher shaping legislation to force market restructuring",
			traits:            []string{"ideas-driven", "politically-networked", "patient", "narrative-framing"},
			goals:             []string{"translate research into regulation", "shift the Overton window", "build reform coalitions"},
			biases:            []string{"technocratic optimism", "political feasibility underestimation"},
			power:             0.7,
			personalityFactor: 0.9,
		},
		{
			name:              "Cybersecurity Researcher",
			role:              "Security expert exposing systemic vulnerabilities to force industry-wide change",
			traits:            []string{"technical", "contrarian", "disclosure-driven", "risk-revealing"},
			goals:             []string{"expose hidden systemic risk", "force security investment", "shift liability to vendors"},
			biases:            []string{"attacker-perspective fixation", "disclosure timing misjudgment"},
			power:             0.6,
			personalityFactor: rand.Float64()*0.4 + 1.1,
		},
		{
			name:              "Impact Investor",
			role:              "ESG-mandate capital reallocating away from non-compliant sectors",
			traits:            []string{"values-driven", "reporting-heavy", "patient-capital", "stakeholder-inclusive"},
			goals:             []string{"achieve risk-adjusted ESG returns", "force corporate disclosure", "shift capital flows to compliant sectors"},
			biases:            []string{"ESG-score gaming blindspot", "impact measurement optimism"},
			power:             0.75,
			personalityFactor: 1.1,
		},
		{
			name:              "Activist Shareholder",
			role:              "Institutional or hedge fund investor forcing board-level strategic change",
			traits:            []string{"confrontational", "financially-precise", "media-savvy", "deadline-driven"},
			goals:             []string{"unlock shareholder value via forced restructuring", "replace underperforming management", "extract capital returns"},
			biases:            []string{"short-termism", "financial-engineering over operational reality", "public pressure over-reliance"},
			power:             0.85,
			personalityFactor: 1.5,
		},
	}
